...
const SubscriptionCard = ({ name, price, icon, billing, currency, color, category, plan, expanded,renewalDate, paymentMethod,startDate, status, onPress}: SubscriptionCardProps) => {
    console.log("expanded", expanded);
  return (
    <Pressable onPress={onPress} 
    className={ clsx("sub-card", expanded ? "sub-card-collapsed" : "bg-card") } style={ !expanded && color? { backgroundColor: color } : undefined}>
        <View className='sub-head'>
            <View className='sub-main'>
                <Image source={icon} className='sub-icon' style={{ width: 32, height: 32 }} />
            ...
            <View className='sub-bdy'>
                ...
                <View className='sub-row'>
                    <View className='sub-row-copy'>
                        ...
                        <Text className="sub-value" numberOfLines={1} ellipsizeMode='tail'>{renewalDate?.trim()}</Text>
